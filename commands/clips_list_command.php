...
class Clips_List_Command extends Clips_Command {
	...
	private function fetchCommands($dir) {
		...
		foreach ($iterator as $info) {
			if ($info->isFile() && $info->getExtension() == 'php') {
				...
				return array_merge($commands, $this->fetchCommands($info->getPathname()));
			}
		}
		return $commands;
	}
	...
	public function execute($args) {
		...
		foreach($tool->listLoadDirs() as $dir) {
			foreach($this->fetchCommands($dir) as $d) {
				$command = str_replace('clips_', '', substr($d, strlen($dir) + 1, strlen($d) - strlen($dir) - 13));
