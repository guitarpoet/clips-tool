...
(function($){
	...
	$.fn.picture = function(args){
		...
		this.each(function(){
			...
			// For auto resizing Added by Jack Thu Aug 14 10:57:47 2014
			var path = $(this).attr('path'); // The auto responsive path
			var src = $(this).attr('src'); // The auto responsive path
			// End for auto resizing

			// Add active state by Andy September 5 15:18:22 2014
			if($(this).hasClass('active')) {
				imgName = src.replace('.png','');
				src = src.replace(imgName, imgName + '-active'); 
			}
			...
			function setPicture(){
				...
				// For auto resizing Added by Jack Thu Aug 14 15:15:17 2014
				if(path) { // If we're using auto resizing
						sizes[currentMedia] = path + '/' + element.width() + '/' + src;
				}
				// End for auto resizing
			...
			function setFigure(){
				...
				// For auto resizing Added by Jack Thu Aug 14 10:59:20 2014
				if(path) { // If we're using auto resizing
						sizes[currentMedia] = path + '/' + element.width() + '/' + src;
				}
				// End for auto resizing
